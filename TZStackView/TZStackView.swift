...
public class TZStackView: UIView {
    ...
    public private(set) var arrangedSubviews: [UIView] = [] {
        didSet {
            setNeedsUpdateConstraints()
            registerHiddenListeners(previousArrangedSubviews: oldValue)
        }
    }
    ...
    private func registerHiddenListeners(previousArrangedSubviews: [UIView]) {
        for subview in previousArrangedSubviews {
            self.removeHiddenListener(view: subview)
        }

        for subview in arrangedSubviews {
            self.addHiddenListener(view: subview)
        }
    }
  ...
  public override func observeValue(forKeyPath keyPath: String?, of object: AnyObject?, change: [NSKeyValueChangeKey : AnyObject]?, context: UnsafeMutablePointer<Void>?) {
        if let view = object as? UIView, change = change where keyPath == "hidden" {
            ...
            let previousValue = change["old" as NSKeyValueChangeKey] as! Bool
            ...
            removeHiddenListener(view: view)
            view.isHidden = false
            ...
            if let _ = view.layer.animationKeys() {
                ...
                didFinishSettingHiddenValue(arrangedSubview: view, hidden: hidden)
            }
        }
    }
    ...
    private func didFinishSettingHiddenValue(arrangedSubview: UIView, hidden: Bool) {
        ...
        addHiddenListener(view: arrangedSubview)
    }
    ...
    func hiddenAnimationStopped() {
        ...
        for entry in animationDidStopQueueEntries {
            ...
            if view.layer.animationKeys() == nil {
                didFinishSettingHiddenValue(arrangedSubview: view, hidden: entry.hidden)
    ...
    override public func willRemoveSubview(_ subview: UIView) {
        removeArrangedSubview(view: subview)
    }
    ...
    override public func updateConstraints() {
        ...
        if arrangedSubviews.count > 0 {
            ...
            case .fillEqually, .fill, .fillProportionally:
                if alignment != .center || layoutMarginsRelativeArrangement {
                    _ = addSpacerView()
                }

                stackViewConstraints += createMatchEdgesContraints(views: arrangedSubviews)
                ...
                if distribution == .fillEqually {
                    stackViewConstraints += createFillEquallyConstraints(views: arrangedSubviews)
                }
                if distribution == .fillProportionally {
                    stackViewConstraints += createFillProportionallyConstraints(views: arrangedSubviews)
                }
                
                stackViewConstraints += createFillConstraints(views: arrangedSubviews, constant: spacing)
            case .equalSpacing:
                var views = [UIView]()
                var index = 0
                for arrangedSubview in arrangedSubviews {
                ...
                stackViewConstraints += createFillConstraints(views: views, constant: 0)
                stackViewConstraints += createFillEquallyConstraints(views: spacerViews)
                stackViewConstraints += createFillConstraints(views: arrangedSubviews, relatedBy: .greaterThanOrEqual, constant: spacing)
            case .equalCentering:
                ...
                stackViewConstraints += createFillEquallyConstraints(views: spacerViews, priority: 150)
                stackViewConstraints += createFillConstraints(views: arrangedSubviews, relatedBy: .greaterThanOrEqual, constant: spacing)
            }
            
            if spacerViews.count > 0 {
                stackViewConstraints += createSurroundingSpacerViewConstraints(spacerView: spacerViews[0], views: visibleArrangedSubviews)
            }
